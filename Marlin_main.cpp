        ...
        case 403: //! M403 set filament type (material) for particular extruder and send this information to mmu
          {
            ...
            if (mmu_enabled)
            {
              ...
              if (code_seen('F')) mmu_filament_types[extruder] = code_value();
              mmu_command(MMU_CMD_F0 + extruder);
...
void manage_inactivity(bool ignore_stepper_queue/*=false*/) //default argument set in Marlin.h
{
  ...
  if (mmu_enabled == false)
  {
    ...
    if (fsensor_check_autoload()) {
      adviseMMUFilamentSeenatExtruder();
    }
  } else {
    if ((mcode_in_progress != 600) && (!moves_planned() && !IS_SD_PRINTING &&
